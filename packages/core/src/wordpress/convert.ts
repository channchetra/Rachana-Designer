...
// Zero-dependency HTML + CSS → GreenShift block markup.
//
// TypeScript port of the original extension's `src/wordpress/convert.js`.
// Every algorithm, regex and generated block-markup template below is
// byte-identical to that file; only the module wrapper (`module.exports`) and
// the types changed, so the converter can run in the browser with no
// build-time Node.js. Handlers that the original wrote against dynamically
// built objects stay loosely typed on purpose.

'use strict';

// ─── Types (for the places the original was dynamically typed) ───────────────

/** A parsed HTML node. Text nodes carry `content`, elements carry `children`. */
interface HTMLNode {
  type: 'element' | 'text';
  tagName: string;
  attributes: Record<string, string>;
  children: HTMLNode[];
  /** Full source of the node, only filled in for skipped-through `<svg>`. */
  outerHTML: string;
  /** Trimmed text, only for text nodes. */
  content: string;
  /** Raw inner source of an `<svg>`/raw-text element (script, style). */
  rawInner?: string;
  rawText?: string;
}

/** One tag token produced by `readTag()`. */
interface TagToken {
  type: 'open' | 'close' | 'comment' | 'doctype';
  tagName: string;
  attributes: Record<string, string>;
  selfClosing: boolean;
  content: string;
...
/** Block attribute bag. The original built this object dynamically. */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type BlockParams = Record<string, any>;

/**
 * Style attribute bag. Values are usually `[value]` arrays (what the Gutenberg
 * block expects) but `customCSS_Extra` is a plain string, so the original
 * mixed both freely — kept loose to match.
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type StyleAttributes = Record<string, any>;

/** One class discovered in the page CSS. */
interface ParsedClassData {
  baseStyles: StyleAttributes;
  baseCSS: string;
  selectors: Array<{
    value: string;
    attributes: { styleAttributes: StyleAttributes };
    css: string;
  }>;
  mediaCSS: string;
  customCSS_Extra: string;
}

/** The whole parsed stylesheet. */
interface ParsedCss {
  classes: Record<string, ParsedClassData>;
  allClasses: Set<string>;
  nonClassCss: string;
}

export interface ConvertOptions {
  editableClasses?: boolean;
}
...
function parseHTML(html: string): HTMLNode[] {
  ...
  function advance(n?: number) { pos += (n || 1); }
  ...
  function readUntil(stop: string) {
    const start = pos;
  ...
  function readAttribute(): { name: string; value: string } | null {
    skipWhitespace();
    let name = '';
  ...
  function readTag(): TagToken | null {
    pos++; // skip <
    ...
    if (html[pos] === '/') {
      ...
      pos++; // skip >
      return { type: 'close', tagName, attributes: {}, selfClosing: true, content: '' };
    }
    // Check for comment
    if (html.slice(pos, pos + 3) === '!--') {
      ...
      pos += 3;
      return { type: 'comment', tagName: '', attributes: {}, selfClosing: true, content };
    }
    // Check for doctype
    if (html.slice(pos, pos + 1).toUpperCase() === '!' || html.slice(pos, pos + 7).toLowerCase() === 'doctype') {
      readUntil('>'); pos++;
      return { type: 'doctype', tagName: '', attributes: {}, selfClosing: true, content: '' };
    }
    // Open tag
    let tagName = '';
    while (remaining() && !/[\s/>]/.test(html[pos])) {
    ...
    return { type: 'open', tagName, attributes, selfClosing, content: '' };
  }
  ...
  function parseNodes(): HTMLNode[] {
    ...
    while (remaining()) {
      if (html[pos] === '<') {
        ...
        if (text.trim()) {
          nodes.push({ type: 'text', tagName: '', attributes: {}, children: [], outerHTML: '', content: text.trim() });
        }
      }
    }
    return nodes;
...
function splitCssValueParts(value: string): string[] {
  const parts: string[] = [];
  let current = '';
  let parenDepth = 0;
...
function parseSpacingShorthand(value: string, prefix: string): StyleAttributes {
  const parts = splitCssValueParts(value);
  const r: StyleAttributes = {};
  if (parts.length === 1) {
...
function parseGapShorthand(value: string): StyleAttributes {
  const parts = splitCssValueParts(value);
  const r: StyleAttributes = {};
...
function parseDeclarations(declarations: string): { styleAttributes: StyleAttributes; customCSS: string } {
  const styleAttributes: StyleAttributes = {};
  const unsupportedProps: Array<{ property: string; value: string }> = [];
...
function extractTopLevelImportRules(cssText: string): { imports: string; remaining: string } {
  ...
  for (let index = 0; index < cssText.length; index++) {
    ...
    if (char !== '@' || braceDepth !== 0 || cssText.slice(index, index + 7).toLowerCase() !== '@import') {
      continue;
    }

    remaining += cssText.slice(chunkStart, index);
    ...
    while (end < cssText.length) {
      ...
      if (importQuote) {
        ...
        if (importChar === importQuote) importQuote = null;
        end++;
        continue;
      }

      if (importChar === '"' || importChar === "'") {
        importQuote = importChar;
        end++;
        continue;
      }
      ...
      if (importChar === ')' && importParenDepth > 0) {
        importParenDepth--;
        end++;
        continue;
      }
      ...
      if (importChar === ';' && importParenDepth === 0) break;
    }

    imports.push(cssText.slice(index, end).trim());
    chunkStart = end;
    index = end - 1;
  }

  remaining += cssText.slice(chunkStart);
...
function parseCss(cssText: string): ParsedCss {
  ...
  const { imports, remaining } = extractTopLevelImportRules(clean);
  if (imports) result.nonClassCss += imports;
  clean = remaining;
  ...
  while ((m = ruleRegex.exec(cssNoMedia)) !== null) {
    ...
    if (isBase) {
      ...
      selectorParts.forEach(part => {
        ...
        const selectorAttrs: StyleAttributes = { ...styleAttributes };
...
function getTypeFromTag(tag: string): string {
...
function getDirectTextContent(node: HTMLNode): string {
...
function hasElementChildren(node: HTMLNode): boolean {
  if (!node.children) return false;
  return node.children.some(c => c.type === 'element');
}

function shouldIncludeDynamicAttribute(name: string, consumedDataType: boolean): boolean {
  if (name === 'data-type' && consumedDataType) return false;
  return (
...
function convertNodeToBlock(node: HTMLNode): string | null {
  if (node.type === 'text') {
    const text = node.content.trim();
    if (!text) return null;
    const params: BlockParams = { textContent: text, tag: 'span' };
  ...
  // Handle br
  if (tag === 'br') {
    const params: BlockParams = { tag: 'span', type: 'html', textContent: '<br />' };
    return `<!-- wp:greenshift-blocks/element ${wpJsonEncode(params)} -->\n<span><br /></span>\n<!-- /wp:greenshift-blocks/element -->`;
  }

  // Handle hr
  if (tag === 'hr') {
    const params: BlockParams = { tag: 'span', type: 'html', textContent: '<hr />' };
    return `<!-- wp:greenshift-blocks/element ${wpJsonEncode(params)} -->\n<span><hr /></span>\n<!-- /wp:greenshift-blocks/element -->`;
  }
  ...
  // data-*, aria-*, event, and selected passthrough attributes -> dynamicAttributes
  ...
  const dynAttrs: Array<{ name: string; value: string }> = [];
  for (const [name, value] of Object.entries(node.attributes)) {
    if (shouldIncludeDynamicAttribute(name, consumedDataType)) {
  ...
  // Iframe attributes — normalize YouTube URLs to embed form; push remaining
  // iframe attrs through dynamicAttributes so the block re-applies them.
  if (tag === 'iframe') {
...
function wpJsonEncode(obj: unknown): string {
...
function buildStyleManagerBlock(
  parsedCss: ParsedCss,
  customJs: string,
  customJsEnabled: boolean,
  { editableClasses = true, rawCss = '' }: { editableClasses?: boolean; rawCss?: string } = {}
): string {
  const params: BlockParams = { tag: 'div', type: 'no', isVariation: 'stylemanager' };

  if (editableClasses) {
    const dynamicGClasses: BlockParams[] = [];

    parsedCss.allClasses.forEach(className => {
      const classData = parsedCss.classes[className];
      const classAttributes: { styleAttributes: StyleAttributes } = { styleAttributes: classData.baseStyles || {} };
    ...
    if (rawCss) params.customCss = rawCss;
  }
...
export function convert(htmlInput: string, options: ConvertOptions = {}): string {
  const { editableClasses = true } = options;
  ...
  function collectMeta(nodeList: HTMLNode[]): void {
  ...
  function findBodyNodes(nodeList: HTMLNode[]): HTMLNode[] | null {
    for (const node of nodeList) {
      if (node.type === 'element') {
  ...
  if (combinedCss || combinedJs) {
    const parsedCss: ParsedCss = combinedCss ? parseCss(combinedCss) : { classes: {}, allClasses: new Set<string>(), nonClassCss: '' };
    styleManagerBlock = buildStyleManagerBlock(parsedCss, combinedJs, !!combinedJs, { editableClasses, rawCss: combinedCss });
  }
...
export function extractCssVariables(cssText: string): Array<{ name: string; value: string }> {
  const variables: Array<{ name: string; value: string }> = [];
  const clean = cssText.replace(/\/\*[\s\S]*?\*\//g, '');
  const varRuleRegex = /(?::root|body|html)\s*\{([^}]*)\}/g;
  let match: RegExpExecArray | null;
  while ((match = varRuleRegex.exec(clean)) !== null) {
    const varRegex = /(--[a-zA-Z0-9_-]+)\s*:\s*([^;]+)/g;
    let varMatch: RegExpExecArray | null;
    while ((varMatch = varRegex.exec(match[1])) !== null) {
      variables.push({ name: varMatch[1].trim(), value: varMatch[2].trim() });
    }
  }
  return variables;
}
...
export function extractMediaUrls(html: string): string[] {
  ...
  const mediaSrcRegex = /<(?:video|audio|source)[^>]+src\s*=\s*["']([^"']+)["']/gi;
  while ((m = mediaSrcRegex.exec(html)) !== null) {
    if (!isExternal(m[1]) && !seen.has(m[1])) { seen.add(m[1]); urls.push(m[1]); }
  }
  return urls;
}

export function replaceMediaUrls(content: string, urlMap: Map<string, string>): string {
  if (urlMap.size === 0) return content;
  // Sort longest key first so that e.g. "./img.webp" is matched before "img.webp",
  // preventing the shorter key from matching inside an already-replaced absolute URL.
